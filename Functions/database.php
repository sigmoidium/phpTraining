<?php
    ...
    function create_tables($db)
    {
        ...
        $db->query($query_users);
		
		$query_products = "CREATE TABLE IF NOT EXISTS products (id INT NOT NULL AUTO_INCREMENT, product VARCHAR(50), image VARCHAR(50), price FLOAT(40), category VARCHAR(50), description VARCHAR(255), PRIMARY KEY (id))";
        $db->query($query_products);

        $query_roles = "CREATE TABLE IF NOT EXISTS roles (id INT NOT NULL, name VARCHAR(50), PRIMARY KEY (id))";
        $db->query($query_roles);
        ...
        $db->query($query_pages);
    }

    function create_roles($db, $admin_role_id)
    {
        $query_check_roles_exist = "SELECT id FROM roles WHERE id <= 2";
        $statement_check_roles_exist = $db->prepare($query_check_roles_exist);
        ...
        if ($statement_check_roles_exist->num_rows == 0)
        {
            ...
            $statement_inser_roles = $db->prepare($query_insert_roles);
            $statement_inser_roles->execute();
        }
    }

    function create_admin($db, $admin_role_id)
    {
        ...
        $statement_check_admin_exists = $db->prepare($query_check_admin_exists);
        ...
        if($statement_check_admin_exists->num_rows == 0)
        {
            ...
            $statement_insert_admin = $db->prepare($query_insert_admin);
            ...
            $statement_add_admin_to_role = $db->prepare($query_add_admin_to_role);
